...
// Removi as lógicas da Meta para focar apenas e exclusivamente no Google Ads
export async function syncMetaStructure(userId: number) { return { businesses: 0, accounts: 0, errors: [] }; }
export async function syncMetaCampaigns(userId: number, accountId: string, adAccountDbId?: number) { return { synced: 0, errors: [] }; }
export async function syncSelectedAccounts(userId: number) { return { synced: 0, errors: [] }; }
...
export async function syncGoogleAdsCampaigns(userId: number, customerId: string): Promise<{ synced: number; errors: string[] }> {
  ...
  try {
    ...
    throw new Error(`Google Ads API: ${e.response?.data?.error?.message || e.message}`); 
  }
...
export async function snapshotAllCampaigns(userId: number): Promise<void> {
  ...
  for (const c of userCampaigns) {
    await saveCampaignSnapshot(c.id, { 
      ...
      conversions: c.conversions, reach: c.reach, ctr: c.ctr, 
      cpc: c.cpc, cpm: c.cpm, roas: c.roas, frequency: c.frequency 
    });
  }
}
