 ...
 * Inclui limpeza de URL para compatibilidade com Neon/Render e força SSL.
 */
export async function getDb() {
  if (!_db && process.env.DATABASE_URL) {
    try {
      // 1. Limpeza da URL: Remove parâmetros que causam erro no driver postgres.js (como channel_binding)
      let dbUrl = process.env.DATABASE_URL;
      if (dbUrl.includes('?')) {
        dbUrl = dbUrl.split('?')[0];
      }

      // 2. Conexão forçando SSL (obrigatório para Neon)
      _client = postgres(dbUrl, { ssl: 'require' });
      _db = drizzle(_client);
    } catch (error) {
...
export async function upsertAdAccount(userId: number, data: { externalId: string; name: string; platform: string; currency?: string; accountStatus?: number; clientId?: number; metadata?: any }) {
  ...
  if (existing.length > 0) {
    const result = await db.update(adAccounts).set({ 
      name: data.name, 
...
export async function upsertCampaign(userId: number, data: any) {
  ...
  if (existing.length > 0) {
    const result = await db.update(campaigns).set({ 
      ...
      updatedAt: new Date(), 
      syncedAt: new Date() 
    }).where(eq(campaigns.id, existing[0].id)).returning();
    return result[0];
  }
...
export async function getDashboardStats(userId: number, days: number = 30) {
  ...
  const avgRoas = userCampaigns.length > 0 
    ? userCampaigns.reduce((s, c) => s + parseFloat(c.roas?.toString() || "0"), 0) / userCampaigns.length 
    : 0;
  ...
  return { 
    totalSpent, totalImpressions, totalClicks, totalConversions, 
    totalReach, avgCtr, avgCpc, avgRoas, byPlatform, 
    campaigns: userCampaigns, clientStats 
  };
}
...
export async function getClientStats(userId: number, days: number = 30) {
  ...
  for (const client of userClients) {
    const clientCampaigns = await db.select().from(campaigns).where(and(
...
export async function getCampaignHistory(campaignId: number, days: number = 30) {
  ...
  if (!db) return [];
  const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
  return db.select().from(campaignHistory)
    .where(and(eq(campaignHistory.campaignId, campaignId), gte(campaignHistory.recordedAt, since)))
    .orderBy(desc(campaignHistory.recordedAt));
}
...
export async function saveIntegrationCredentials(data: any) {
  ...
  if (existing.length > 0) {
    const result = await db.update(integrationCredentials).set({ 
      ...data, 
      updatedAt: new Date() 
    }).where(eq(integrationCredentials.id, existing[0].id)).returning();
    return result[0];
  }
